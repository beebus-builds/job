...
import { ArrowRight, Bot, BriefcaseBusiness, Building2, CheckCircle2, Sparkles, Workflow } from 'lucide-react'
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#08090d] text-white">
      ...
      <nav className="relative mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-10">
        ...
        <Link href="/dashboard" className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm hover:bg-white/10">Open app</Link>
      </nav>
      ...
      <section className="relative mx-auto max-w-7xl px-6 pb-24 pt-16 lg:px-10 lg:pt-24">
        <div className="max-w-4xl">
          ...
          <div className="mt-10 flex flex-wrap gap-3"><Link href="/dashboard" className="group flex items-center gap-2 rounded-full bg-white px-6 py-3 font-medium text-black">Find jobs <ArrowRight size={17} className="transition group-hover:translate-x-1"/></Link><Link href="/employer/jobs/new" className="rounded-full border border-white/15 px-6 py-3 text-zinc-200 hover:bg-white/5">Post a role</Link></div>
        </div>

        <div className="mt-20 grid gap-4 md:grid-cols-3"><Metric label="Candidate matches" value="94%" detail="AI-ranked against your profile" /><Metric label="Hiring workflow" value="24/7" detail="Automations run in the background" /><Metric label="One marketplace" value="2-sided" detail="Candidates + employers together" /></div>
      </section>

      <section id="how" className="relative border-y border-white/10 bg-white/[.025] px-6 py-24 lg:px-10"><div className="mx-auto max-w-7xl"><p className="text-sm uppercase tracking-[.2em] text-zinc-500">One system · two agents</p><h2 className="mt-4 max-w-3xl text-4xl font-semibold tracking-tight sm:text-5xl">Your job search and your hiring pipeline should run themselves.</h2><div className="mt-14 grid gap-5 lg:grid-cols-2"><Panel id="seekers" icon={<BriefcaseBusiness/>} eyebrow="For job seekers" title="Your career agent" copy="Tell jobOS what you want. It discovers, understands, ranks and prepares opportunities around your actual experience." steps={seekerSteps}/><Panel id="employers" icon={<Building2/>} eyebrow="For employers" title="Your hiring agent" copy="Define the outcome. jobOS turns the role into a hiring workflow, screens candidates and keeps the pipeline moving." steps={employerSteps}/></div></div></section>
